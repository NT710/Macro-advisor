...
- **Access:** WebFetch on `https://themacrocompass.substack.com/archive` → get recent article URLs → WebFetch each article. **Note:** The Macro Compass has a mix of free and paid articles. If an article returns partial content with a paywall message (e.g., "Come join The Macro Compass premium platform" or "subscribe to read"), extract whatever is visible in the free preview, note it as "[paywalled — preview only]", and move on. Do not retry paywalled articles. If a Substack newsletter signup modal appears, dismiss it (click "No thanks" or navigate past it) — the content is behind the modal, not blocked by it.
...
- v2.1 (2026-W13): Fixed Macro Compass access instructions. Root cause: WebFetch was reported as "failing" on Substack, but investigation showed it works fine — the real issue is some articles are paywalled (free preview + "subscribe to read" cutoff). Updated access method to use `/archive` page for article discovery and handle paywalled articles gracefully (extract preview, note as paywalled, move on). Also added newsletter signup modal dismissal instruction. Supersedes proposed amendment A-2026W12-005.
