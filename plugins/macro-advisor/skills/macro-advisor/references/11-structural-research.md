...
4. **Flagged by Skill 14 from decade-horizon blind spots.** When Skill 14 (Decade Horizon Strategic Map) identifies a blind spot in the thesis book with timeline <3 years and available data, it flags it for Skill 11 investigation via `outputs/strategic/blind-spots/`. These enter Skill 7's investigation queue alongside other candidates and share the 5-investigation cap. The blind spot file contains the mega-force context, causal chain, and consensus assessment — use these as the strategic framing, then apply the same first-principles research methodology as for any other candidate.
5. **Manual invocation.** The user identifies a structural theme worth investigating. No weekly chain dependency required.
