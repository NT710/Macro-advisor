...
8. **Write monitoring results back to the thesis.** Do this for each thesis immediately after finishing its monitoring — not as a batch at the end. Three separate steps, all mandatory:
   ...
   **Step 8a — JSON sidecars are compiled automatically.** Do NOT write `-data.json` files manually. The `compile_sidecars.py` script runs after Skill 7 completes and deterministically extracts JSON from the markdown you write. This eliminates context-pressure degradation (truncation, missing fields, placeholder text) that occurred when sidecars were generated by the LLM.

   **Your only responsibility is to write complete, well-structured markdown** using the standard heading template. The compiler parses these headings to produce the sidecar:
   - `## Summary` → `summary`
   - `## The Bet` → `the_bet`
   - `### Mechanism` (numbered list) → `mechanism[]`
   - `## What Has To Stay True` (numbered list) → `what_has_to_stay_true[]`
   - `## Where The Market Stands` → `where_the_market_stands`
   - `## The Trade` with sub-headings → `the_trade` object
   - `### What Can't Change` → `what_cant_change[]` (structural only)
   - `## What Could Break It` → `what_could_break_it` (structural only)

   **Every thesis MUST use these exact section headings.** If a heading is missing, the corresponding sidecar field will be null or empty, and the dashboard will have a gap. The `postrun_check.py` fidelity verification will catch count mismatches and missing sections.

   **Conviction** must appear somewhere in the markdown as `*Thesis conviction:* Level` or `**Conviction:** Level` where Level is one of: High, Medium-High, Medium, Low-Medium, Low. Do not add parenthetical qualifiers (e.g., "Medium (tactical)") — use a clean level only.

   The `change_log` is generated automatically by `compile_sidecars.py` by diffing the current sidecar against the previous version. No manual changelog maintenance needed.
