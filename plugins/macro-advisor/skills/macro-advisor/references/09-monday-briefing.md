...
## Dashboard Data JSON (compiled automatically)

**Do NOT write `briefing-data.json` manually.** The `compile_briefing.py` script runs after Skill 9 completes and deterministically extracts structured JSON from the synthesis markdown and compiled thesis sidecars. This eliminates key naming inconsistencies (`asset` vs `what`, `signal` vs `direction`, `etf` vs `etfs`) and ensures all theses on disk are included.

**Path:** `outputs/briefings/{week}-briefing-data.json` — generated by `compile_briefing.py`

**Your responsibility:** Write the narrative briefing memo. The cross-asset table in the synthesis (Skill 6) and the sector view prose are parsed by the compiler. Ensure the synthesis has a properly formatted markdown table under `## Cross-Asset Implications` and prose sectors under `## Sector View`.

The following JSON schema documents what `compile_briefing.py` produces (for reference — you do not need to write this):
