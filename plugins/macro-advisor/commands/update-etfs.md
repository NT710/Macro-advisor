...
3. If preferred currency is USD, no currency-specific override is needed — the base `${CLAUDE_PLUGIN_ROOT}/skills/macro-advisor/references/etf-reference.md` already covers USD directly. Just verify tickers are still active via Yahoo Finance and report any delistings.
4. If preferred currency is CHF, EUR, or GBP, regenerate `config/etf-overrides.md` in the workspace. Cover comprehensively:
...
5. **Do not modify** the base `${CLAUDE_PLUGIN_ROOT}/skills/macro-advisor/references/etf-reference.md` — it contains universal USD sections and is read-only plugin code.
