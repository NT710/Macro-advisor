...
- `skills/references/etf-reference.md` — ETF lookup tables (broad, thematic, CHF equivalents).
...
`weekly-macro-advisor` runs Sundays at 16:00 CET. Execution order: 0→1→2→3→4→5→10→6→7→11(if triggered)→8→12→9. Delivers an HTML dashboard.
...
4. ETF-focused, CHF-listed where available. Theses use first/second/third-order ETF chains verified via etf_lookup.py.
